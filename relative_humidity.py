...
def relative_humidity(buffer = 0, dataFile = 'str') -> None:
    ...
    for i in range((end_date - start_date).days):
        ...
        file = "map/relative_humidity/" + fileName + day.strftime('%Y%m%d') + ".tif"
        _map = map()
        _map.setMap(file)
        for data in dataStationArray:
            if((data[0] == day.strftime('%Y-%m-%d'))):
                ...
                data[9] = np.float64(_station.bufferValue)
...
relative_humidity(1, 'buffer_1_data.csv')
relative_humidity(2, 'buffer_2_data.csv')
relative_humidity(3, 'buffer_3_data.csv')