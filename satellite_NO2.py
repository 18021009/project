...
def satellite_NO2(buffer = 0, dataFile = 'str') -> None:
    ...
    for i in range((end_date - start_date).days):
        ...
        file = "map/satellite_NO2/" + fileName + day.strftime('%Y%m%d') + ".tif"
        _map = map()
        _map.setMap(file)
        for data in dataStationArray:
            if((data[0] == day.strftime('%Y-%m-%d'))):
                ...
                data[7] = np.float64(_station.bufferValue)
...
satellite_NO2(1, 'buffer_1_data.csv')
satellite_NO2(2, 'buffer_2_data.csv')
satellite_NO2(3, 'buffer_3_data.csv')