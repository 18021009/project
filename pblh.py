...
def pblh(buffer = 0, dataFile = 'str') -> None:
    ...
    for i in range((end_date - start_date).days):
        fileName = "HPBLCombine_"
        day = start_date + i*day_delta
        file = "map/pblh/" + fileName + day.strftime('%Y%m%d') + ".tif"
        _map = map()
        _map.setMap(file)
        for data in dataStationArray:
            if((data[0] == day.strftime('%Y-%m-%d'))):
                ...
                data[12] = np.float64(_station.bufferValue)
...
pblh(1, 'buffer_1_data.csv')
pblh(2, 'buffer_2_data.csv')
pblh(3, 'buffer_3_data.csv')