...
def pressure(buffer = 0, dataFile = 'str') -> None:
    ...
    for i in range((end_date - start_date).days):
        fileName = "PRESSCombine_"
        day = start_date + i*day_delta
        file = "map/pressure/" + fileName + day.strftime('%Y%m%d') + ".tif"
        _map = map()
        _map.setMap(file)
        for data in dataStationArray:
            if((data[0] == day.strftime('%Y-%m-%d'))):
                ...
                data[10] = np.float64(_station.bufferValue)
...
pressure(1, 'buffer_1_data.csv')
pressure(2, 'buffer_2_data.csv')
pressure(3, 'buffer_3_data.csv')